...
Regardless of how you get the HTML, you'll need some way to the identify the elements that contain the data you care about.
...
[^2]: Or another element, more on that shortly.
...
For example,The most important combinator is " ", called the descendent combination because `p a` selects all `<a>` elements that are a child of a `<p>` element.
...
In this case, you should generally start by using `html_elements()` to select all observations and `html_element()` to extract the elements that will form the variants.
...
Instead, use `html_elements()` to find a element for each character, and then `html_element()` for each value:
...
`html_element()` automatically fills in `NA` when no elements match, keeping all of the variables in alignment and making it easier to feed them into a data frame:
