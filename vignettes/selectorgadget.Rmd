...
2.  Click on elements that shouldn't be selected.
    They will turn red.
...
For example, imagine we want to find the actors listed on an IMDB movie page, e.g.
...
2.  Click on the SelectorGadget link in the bookmarks.
    The SelectorGadget console will appear at the bottom of the screen, and element currently under the mouse will be highlighted in orange.

    ```{r, echo = FALSE}
    knitr::include_graphics("selectorgadget-2-s.png")
    ```

3.  Click on the element you want to select (the name of an actor).
    The element you selected will be highlighted in green.
    SelectorGadget guesses which css selector you want (`a` in this case), and highlights all matches in yellow (see total count equal to 592 as indicated on on the "Clear" button).
    This seems to be a little too excessive.

    ```{r, echo = FALSE}
    knitr::include_graphics("selectorgadget-3-s.png")
    ```

4.  Scroll around the document to find elements that you don't want to match and click on them.
    For example, we don't to match the character the actor contributed to, so we click on it and it turns red.
    The css selector updates to `.primary_photo+ td a`.
